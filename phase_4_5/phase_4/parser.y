						...
						// if($$->stack){
							...
							if (curr_stack->breaklist)
							 ...
							 if(curr_stack->contlist)
								$$->stack->contlist = merge_list(curr_stack->contlist, bc_head->bc_list->contlist);
						// }
						}
		;		
...
expr:		expr PLUS expr			{
		...
		|	expr EQUAL expr			{
									...
									emit(assign, newexpr_constbool('F'),NULL, $$, nextquadlabel(), yylineno);
									emit(jump, NULL, NULL, NULL, nextquadlabel()+2, yylineno);
									emit(assign, newexpr_constbool('T'), NULL, $$, nextquadlabel(), yylineno);
									resettemp();
									}
...
term : LPARENTHESIS expr RPARENTHESIS { $$ = $2; }
		| MINUS expr %prec UMINUS
		{
			if (checkuminus($2) == 0)
			{
				...
				emit(mul, $2, newexpr_constnum(-1), $$, nextquadlabel(), yylineno);
				resettemp();
			}
			else
			{
