...
tf_stack *push_tf_stack(tf_stack **head, list *c)
{
    tf_stack *new = (tf_stack *)malloc(sizeof(tf_stack));
    new->falselist = (list *)malloc(sizeof(list));
    new->falselist = c;
    if (*head == NULL)
    {
        *head = new;
...
list *pop_tf(tf_stack *head)
{
    ...
    memcpy(&result, &head->falselist, sizeof(list));
    head = head->next;
    tf_head = head;
    // free(tmp);
    return result;
...
void backpatch(list **head, int quad)
{
    list *tmp = *head;
    while (tmp)
    {
        // printf("BACKPATCH: quad= %d, label = %d\n", quad, tmp->label);
        patchlabel(tmp->label, quad);
        // if (tmp->next == NULL)
        // {
        //     break;
        // }
...
expr *to_bool(expr *e)
{
    ...
    {
        e->truelist = (list *)malloc(sizeof(list));
        e->truelist->label = nextquadlabel();
        printf("To bool truelist label = %d\n", e->truelist->label);
        emit(ifeq, e, NULL, newexpr_constbool('T'), -1, yylineno);
        e->falselist = (list *)malloc(sizeof(list));
        e->falselist->label = nextquadlabel();
