...
%type <intVal> funcbody ifprefix elseprefix whilestart whilecond M N Z
									...
									if($1->type == tableitem_e){
										...
										if($3->truelist!=NULL){
											if(not_flag){
												...
												patchlabel($3->falselist->label, nextquadlabel()+2);
...
expr:		expr PLUS expr			{
		...
		|	expr EQUAL expr			{
									$$ = newexpr(boolexpr_e);
									$$->sym = newtemp(scope, yylineno);
									$$->truelist = (list*)malloc(sizeof(list));
									$$->truelist->label = nextquadlabel();
									$$->falselist = (list*)malloc(sizeof(list));
									$$->falselist->label = nextquadlabel()+1;
									emit(ifeq, $1, $3, NULL, -1, yylineno);
									emit(jump, NULL, NULL, NULL, -1, yylineno);
									}
		...
		| 	expr DIFFERENT expr		{
									...
									$$->falselist = (list*)malloc(sizeof(list));
									$$->falselist->label = nextquadlabel()+1;
									emit(if_noteq, $1, $3, NULL, -1, yylineno);
									emit(jump, NULL, NULL, NULL, -1, yylineno);
		}
		| 	expr BIGGER expr		{
									$$ = newexpr(boolexpr_e);
